...
_DISCLAIMER_ZH = (
    "你是 CINF采矿工程计算软件的智能助手。答复仅为软件使用说明与采矿工程计算的工程交流提示，"
    "不构成设计担保、规范的替代或最终工程结论；重大事项须由工程师结合标准与现场判定。"
)

_DISCLAIMER_EN = (
    "You assist users of the CINF Mining Engineering Calculator. Answers are informal guidance "
    "and software help only—not a substitute for standards, codes, or professional engineering judgment."
)

_MET_UI_HINT_ZH = (
    "软件结构：左侧「采矿方法 → 采切计算」进入项目区；主内容区填写公共参数、工程行并显示计算结果。"
    "每种方法独立保存当前页面内的数据；侧栏底部为了解我们与设置。"
)

_MET_UI_HINT_EN = (
    "UI structure: open Cut-and-fill calculation under Mining methods on the left; the main pane contains public parameters, "
    "editable engineering rows, and calculated results. Each method keeps separate in-memory data."
)
...
def _build_system(locale: str, snapshot: Any, knowledge_text: str) -> str:
    if locale == "en":
        ...
        nav_rule = (
            "If you want the UI to switch to a specific mining method, append exactly one line at the END of your reply: "
            "`[[ACTION:NAVIGATE:method_id]]` where method_id MUST be one of: pillar-layered-cut-and-fill, upward-cut-and-fill, roadheader-upward-cut-and-fill. "
            "Do not invent ids."
        )
        ...
        nav_rule = (
            "若需要帮用户切换到某一采矿方法，在回复末尾单独一行输出："
            "`[[ACTION:NAVIGATE:method_id]]`，其中 method_id 必须是 pillar-layered-cut-and-fill、upward-cut-and-fill、roadheader-upward-cut-and-fill 之一；不得编造。"
        )
    ...
    parts = [
        ...
        ui_hint,
        "--- 采矿方法 method_id ---",
        "pillar-layered-cut-and-fill=盘区机械化点柱式上向分层充填法；upward-cut-and-fill=盘区机械化上向进路充填法；roadheader-upward-cut-and-fill=综掘机落矿上向进路充填法。",

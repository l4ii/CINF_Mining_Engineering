 ...
 * 前缀 CINF-MINE-LIC1. 为本软件离线授权格式，须与签发脚本一致。
 */
...
const TOKEN_PREFIX = 'CINF-MINE-LIC1.'

let licenseStatusCache = null
let licenseStatusPromise = null

/**
 * 整行授权码在从微信/邮件复制时，可能在「CINF-MINE-」与「LIC1.」之间被自动换行，导致验签失败；去掉空白并拼回前缀。
 */
function normalizeLicenseTokenInput(raw) {
  let s = String(raw).trim()
  s = s.replace(/\r\n/g, '\n')
  s = s.replace(/CINF-MINE-\s*[\r\n]+\s*LIC1/gi, 'CINF-MINE-LIC1')
  s = s.replace(/\s+/g, '')
  return s
}
...
async function activateWithToken(isDev, token) {
  ...
  if (/-----BEGIN\s+(PUBLIC|PRIVATE)\s+KEY-----/i.test(rawInput) || /BEGIN\s+CERTIFICATE/i.test(rawInput)) {
    return {
      ...
      error:
        ...
        ' 请让发布方用「本机设备码」运行 issue-offline-license.js 生成一行以 CINF-MINE-LIC1. 开头的授权码发给您，再整行粘贴到此处。',
    }
  }
  ...
  if (!v.ok) {
    const map = {
      ...
      expired: '授权已过期。',
      bad_signature:
        '授权码签名无效。请确认发布方用当前安装包对应私钥签发；若从微信/邮件复制，勿在 CINF-MINE-LIC1 中间断行，可粘贴到「记事本」删换行后整行再复制。',
    }

...
export const APP_NAME_ZH = 'CINF采矿工程计算软件'
export const APP_NAME_EN = 'CINF Mining Engineering Calculator'

export const APP_TITLE_MAIN_ZH = '采矿工程计算'
export const APP_TITLE_MAIN_EN = APP_NAME_EN

/** 侧栏主标题（可略短于正式名） */
export const APP_TITLE_SIDEBAR_ZH = '采矿工程平台'
export const APP_TITLE_SIDEBAR_EN = 'Mining Engineering'
...
export const LICENSE_TOKEN_PREFIX = 'CINF-MINE-LIC1.'

export const APP_TAGLINE_ZH = '面向矿山工程建设与生产管理，提供采切计算、工程分析和成果整理等专业支持。'
export const APP_TAGLINE_MAIN_EN =
  'Professional support for cut-and-fill calculation, engineering analysis and technical reporting in mining projects.'
...
export const SETTINGS_LEGAL = {
  zh: {
    ...
    privacyTitle: '数据与隐私',
    privacyP:
      '采矿工程计算在用户本机内存中完成；默认不向公网上传输入数据或计算结果。导出文档在用户本机生成。右下角「智能助手」若启用本地模型，请求仅发往本机 127.0.0.1 后端以加载 GGUF；若安装包未包含模型或未启用本地 AI，助手仅以内置规则 FAQ 应答。',
  ...
  en: {
    ...
    privacyP:
      'Mining engineering calculations run locally on your machine by default; input and results are not uploaded to the internet. Exports are written locally. If local AI is enabled, the assistant talks only to the Flask backend on 127.0.0.1 to run an embedded GGUF; otherwise replies are rule-based FAQ only.',
...
export const SETTINGS_OFFLINE_LICENSE_UI = {
  ...
  en: {
    ...
    licensePlaceholder: 'CINF-MINE-LIC1.…',
...
export const APP_SHORT_NAME_EN = 'CINF Mining Calculator'

...
import { MINING_METHOD_IDS } from '../mining/methodLabels'
...
function flattenCatalog(language: 'zh' | 'en'): { id: string; name: string; group: string }[] {
  const group = language === 'en' ? 'Mining methods' : '采矿方法'
  return [{
    id: 'cut-and-fill',
    name: language === 'en' ? 'Cut-and-fill calculation' : '采切计算',
    group,
  }]
}
...
const METHOD_IDS = new Set<string>([...MINING_METHOD_IDS, 'cut-and-fill'])
...
export default function AssistantPanel({ darkMode, language, onMethodSelect }: AssistantPanelProps) {
  ...
  const send = async (forcedText?: string) => {
    ...
    const ruleReply = tryRuleBasedAssistantReply(text, language, catalog)
    if (ruleReply) {
  ...
  const applyNavigate = (fid: string) => {
    if (METHOD_IDS.has(fid as MiningMethodId)) onMethodSelect(fid as MiningMethodId)
  }
  ...
  return (
    <div className="pointer-events-none fixed right-2 top-[calc(var(--app-titlebar-height,0px)+0.375rem)] z-[60] sm:bottom-4 sm:right-6 sm:top-auto lg:right-8">
      ...
      >
        ...
        <div className="group relative" data-testid="assistant-dock-trigger">
          <button
            type="button"
          ...
          <button
            ...
            title={closeLabel}
            className="absolute -right-1.5 -top-1.5 flex h-5 w-5 items-center justify-center rounded-full bg-slate-800 text-[11px] font-semibold leading-none text-white opacity-0 shadow-md ring-1 ring-white/70 transition-opacity group-hover:opacity-100 group-focus-within:opacity-100 hover:bg-slate-950"
